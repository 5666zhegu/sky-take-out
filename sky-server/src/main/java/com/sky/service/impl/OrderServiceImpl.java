...
//        );
    ...
    public void paySuccess(String outTradeNo) {
        ...
        map.put("content","您有一个新的订单，请尽快处理！订单号：" + outTradeNo);
...
//                    new BigDecimal(0.01));//原订单金额
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        if(page != null && page.getTotal() > 0){
            for (OrderVO orderVO : page) {
                List<OrderDetail> orderDetails = ordersDetailMapper.getByOrderId(orderVO.getId());
                orderVO.setOrderDetailList(orderDetails);
            }
        }
        long total = page.getTotal();
...
//            log.info("申请退款：{}", refund);
    ...
    public void remider(Long id) {
        OrderVO order = orderMapper.getById(id);
        if(order == null && order.getStatus() != TO_BE_CONFIRMED){
            throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
        }
        Map map = new HashMap();
        map.put("type",2);
